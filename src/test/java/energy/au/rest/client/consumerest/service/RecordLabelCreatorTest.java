...
public class RecordLabelCreatorTest {
	...
	public void testCreateRecordLabelStructure() {
		...
		Assert.isTrue(recordLabels.containsKey("Pacific Records1"),
				() -> "Pacific Records1 must be present at the top as key.");
		...
		Assert.isTrue(womenRecBands.contains(jillBlackTest),
				() -> "Fourth Woman Records1 record label must contain Jill Black-1 band.");
		...
		Assert.isTrue(jillBlackReal.getMusicFestivals().size() == 2,
				() -> "Number of music festivals for Jill black must be 2");
	}
}
